...
sendKeystrokesToUBoot () {
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "pleaseDontAutoBoot" -e "PCA-102 #>" 
	RETVAL=$?
...
sendKeystrokesToBarebox() {
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "pleaseDontAutoBoot" -e "barebox@" 
	RETVAL=$?
...
configureBareboxForFlashingNand() {
	...
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "loadenv /dev/defaultenv" -e "loading environment from /dev/defaultenv"
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "saveenv" -e "saving environment"
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "go 0x82000000" -e "## Starting application at 0x82000000" 
...
EOF
	...
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "loadb -f env/config -c" -e "## Ready for binary (kermit) download"
	$BUILDDIR/ukermit -p ${SERIAL_PORT} -f /tmp/barebox-envconfig-adapted
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "saveenv" -e "saving environment"
...
flashNandPartThroughTftp() {
	...
	$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "update -t ${NANDPART} -d nand -f ${IMAGETOWRITE}" -e "barebox@" 
...
$BUILDDIR/pserial -p ${SERIAL_PORT} -f $BASEDIR/x-load-withukermitsupport.bin -v

# Bringing u-boot to xloader over serial line
logMessage "Bringing u-boot over serial line"
$BUILDDIR/ukermit -p ${SERIAL_PORT} -f ${PTXDIST_IMAGES_DIR}/u-boot.bin
...
$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "loadb" -e "## Ready for binary (kermit) download to 0x82000000" 
$BUILDDIR/ukermit -p ${SERIAL_PORT} -f ${PTXDIST_IMAGES_DIR}/barebox-image 
...
# Print version
$BUILDDIR/ucmd -p ${SERIAL_PORT} -c "version" -e "barebox@" 
